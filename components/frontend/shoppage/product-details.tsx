...
// import { Star, Minus, Plus, Heart, Facebook, Twitter, Linkedin } from "lucide-react"
...
//             <Button onClick={handleAddToCart} className="bg-red-600 hover:bg-red-700 text-white flex-1">
...
import { AppDispatch } from "@/redux/store"
import { useDispatch, useSelector } from "react-redux"
import { removeItem, updateqty, addItem } from "@/redux/slices/cartSlice"
...
export default function ProductDetail({ product }: { product: any }) {
  ...
  const handleAddToCart = () => {
    ...
    // Add item to Redux cart with quantity
    dispatch(addItem({ 
      id: product.id,
      title: product.name,
  ...
  const handleIncreaseQuantity = () => {
    if (quantity < (product.qty || 10)) { // Limit by product quantity or default max
      setQuantity(quantity + 1);
    } else {
