...
export default function LatestDeals({products}:{products:any}) {
  ...
  return (
    <div className="w-full">
        ...
        {products.slice(0,4).map((deal:any) => (
          ...
          >
            ...
            <div className="relative mb-4 flex-shrink-0 md:mb-0 md:mr-6">
              <div className="absolute left-0 top-0 z-10 flex h-12 w-12 items-center justify-center bg-green-500 text-sm rounded-xl px-1 font-bold text-white">
              ...
              <div className="relative h-48 w-48 overflow-hidden rounded-md bg-gray-100">
                ...
                >
                  <Heart
                    className={`h-5 w-5 ${wishlist.includes(deal.id) ? "fill-green-500 text-green-500" : "text-gray-400"}`}
                  />
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-1 flex-col">
              ...
              {/* Add to Cart */}
              <Button className="mt-auto w-full bg-green-600 hover:bg-green-700">Add to Cart</Button>
            </div>
          </Link>
        ))}
      </div>
      ...
      {/* Discount Banner */}
      <div className="relative mt-8 overflow-hidden rounded-lg border border-green-100 bg-green-50 p-6">
        <div className="absolute -right-10 -top-10 text-[120px] font-bold text-green-100">-39%</div>
        <div className="relative z-10 flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
          <div>
            <h3 className="text-xl font-bold text-green-500">Super discount for your first purchase</h3>
            <p className="text-sm text-gray-600">Use discount code in checkout page.</p>
          </div>
          <div className="flex items-center gap-4">
            <span className="rounded-md border border-green-200 bg-white px-4 py-2 font-mono text-lg font-bold text-green-500">
              FARM15FIRST
            </span>
